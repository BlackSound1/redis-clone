...
// build builds up various statistics about the Redis server
func (info *Info) build(state *AppState) {
	...
	info.server = map[string]string{
		...
		"executable":        execPath,
		"config_file":       state.conf.config_file,
	}
...
// print prints the various info the INFO call returns
func (info *Info) print(state *AppState) string {
	info.build(state)

	// Build up this message with each category's info
	var msg string = ""

	// Inner function to format the info of a particular category in a nice way
